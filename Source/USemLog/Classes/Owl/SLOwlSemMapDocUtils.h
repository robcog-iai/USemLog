...
*/
UENUM(BlueprintType)
enum class ESLOwlSemMapTemplateTypes : uint8
{
 ...
 */
struct USEMLOG_API FSLOwlSemMapDocUtils
{
public:
	// Steps for creating and printing the semantic map document with default arguments
	static bool CreateAndPrintDoc(UWorld* World, bool bOverwrite, ESLOwlSemMapTemplateTypes Type = ESLOwlSemMapTemplateTypes::Ameva);
...
private:
	...
	static FSLOwlDoc GetDocumentTemplate(ESLOwlSemMapTemplateTypes Type);

	// Add map boilerplate entity definitions depending on the map template
	static void AddEntityDefinitions(FSLOwlDoc& Doc, ESLOwlSemMapTemplateTypes Type);

	// Add map boilerplate namespace declarations depending on the map template
	static void AddNamespaceDeclarations(FSLOwlDoc& Doc, ESLOwlSemMapTemplateTypes Type);

	// Add map ontology imports depending on the map template
	static void AddOntologies(FSLOwlDoc& Doc, ESLOwlSemMapTemplateTypes Type);
	...
	/* Constants */
	static constexpr char AmevaNs[] = "ameva";
	static constexpr char KRNs[] = "knowrob";
};
