...
// Sets default values
ASLIndividualVisualManager::ASLIndividualVisualManager()
{
...
void ASLIndividualVisualManager::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	/* Buttons hack */
	if (PropertyName == GET_MEMBER_NAME_CHECKED(ASLIndividualVisualManager, bToggleTickUpdate))
	{
		bToggleTickUpdate = false;
		ToggleTickUpdate();
	}
...
// Called every frame
void ASLIndividualVisualManager::Tick(float DeltaTime)
{
...
bool ASLIndividualVisualManager::ShouldTickIfViewportsOnly() const
{
	return true;
}

// Load components from world
int32 ASLIndividualVisualManager::Init(bool bReset)
{
	if (bReset)
	{
		bIsInit = false;
...
USLIndividualTextInfoComponent* ASLIndividualVisualManager::GetInfoComponent(AActor* Actor) const
{
	if (UActorComponent* AC = Actor->GetComponentByClass(USLIndividualTextInfoComponent::StaticClass()))
	{
		...
		{
			return CastChecked<USLIndividualTextInfoComponent>(AC);
		}
	}
	return nullptr;
}
...
USLIndividualTextInfoComponent* ASLIndividualVisualManager::AddNewInfoComponent(AActor* Actor)
{
	...
	if (!HasInfoComponent(Actor) && HasIndividualComponentSibling(Actor))
	{
		...
		FName NewComponentName = *FComponentEditorUtils::GenerateValidVariableName(
			USLIndividualTextInfoComponent::StaticClass(), Actor);
		...
		USLIndividualTextInfoComponent* NewComp = NewObject<USLIndividualTextInfoComponent>(Actor, NewComponentName, RF_Transactional);
...
bool ASLIndividualVisualManager::HasInfoComponent(AActor* Actor) const
{
	return Actor->GetComponentByClass(USLIndividualTextInfoComponent::StaticClass()) != nullptr;
}

// Info component can be created if there is an individual component present
bool ASLIndividualVisualManager::HasIndividualComponentSibling(AActor* Actor) const
{
	return Actor->GetComponentByClass(USLIndividualComponent::StaticClass()) != nullptr;
}

// Removes component from the actors instanced components, triggers destroy
void ASLIndividualVisualManager::DestroyInfoComponent(USLIndividualTextInfoComponent* Component)
{
...
bool ASLIndividualVisualManager::RegisterInfoComponent(USLIndividualTextInfoComponent* Component)
{
	bool bRetVal = true;
	...
	if (!InfoComponentOwners.Contains(CompOwner))
	{
		...
		if (!CompOwner->OnDestroyed.IsAlreadyBound(this, &ASLIndividualVisualManager::OnSemanticOwnerDestroyed))
		{
			CompOwner->OnDestroyed.AddDynamic(this, &ASLIndividualVisualManager::OnSemanticOwnerDestroyed);
		}
	}
	else
	{
	...
	if (!Component->OnDestroyed.IsAlreadyBound(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed))
	{
		Component->OnDestroyed.AddDynamic(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed);
	}
	else
	{
...
bool ASLIndividualVisualManager::UnregisterInfoComponent(USLIndividualTextInfoComponent* Component)
{
	bool bSuccess = true;
	...
	if (Component->OnDestroyed.IsAlreadyBound(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed))
	{
		Component->OnDestroyed.RemoveDynamic(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed);
	}
	else
	{
...
int32 ASLIndividualVisualManager::ClearCache()
{
	int32 Num = 0;
	for (const auto& C : RegisteredInfoComponents)
	{
		...
		if (C->OnDestroyed.IsAlreadyBound(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed))
		{
			C->OnDestroyed.RemoveDynamic(this, &ASLIndividualVisualManager::OnInfoComponentDestroyed);
			Num++;
		}
		else
		{
...
void ASLIndividualVisualManager::OnInfoComponentDestroyed(USLIndividualTextInfoComponent* DestroyedComponent)
{
...
void ASLIndividualVisualManager::OnSemanticOwnerDestroyed(AActor* DestroyedActor)
{
