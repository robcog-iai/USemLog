...
// Load semantic data
bool USLSkyIndividual::Load(bool bReset, bool bTryImportFromTags)
{
	if (bReset)
	{
		LoadReset();
...
bool USLSkyIndividual::ApplyMaskMaterials(bool bReload)
{
	if (!IsInit())
	{
		return false;
...
bool USLSkyIndividual::LoadImpl(bool bTryImportFromTags)
{
	return true;
}
