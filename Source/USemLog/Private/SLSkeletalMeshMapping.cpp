...
void USLSkeletalMeshMapping::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(USLSkeletalMeshMapping, Skeleton))
	{
		UE_LOG(LogTemp, Warning, TEXT("\t %s::%d"), TEXT(__FUNCTION__), __LINE__);
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(USLSkeletalMeshMapping, SkeletalMesh))
	{
